...
10. 如果评价、晒单、评论任务处于第 1 步，且还没有输入内容，不要点击发送/发布/提交/返回；应先点击评价入口、晒单入口、商品评价入口或评论入口。
11. 刚输入评价或评论后，不要点右上角搜索/完成图标；社交评论应点右下角发送，电商评价无明确发布按钮时直接 complete。
12. 如果出现权限、广告、青少年模式、升级、弹窗，优先点击关闭、跳过、取消或允许继续任务的按钮。
13. 不要重复点击最近历史里连续点过的位置。
14. 只有已经达到用户任务目标后才 complete；搜索结果页、详情页、输入后未发送都不能提前 complete。
15. 如果目标控件当前屏幕不可见，才使用 scroll；首屏已有可点击目标时不要 scroll。
